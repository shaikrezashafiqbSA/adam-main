...
db_telegram_cache_path = "./database/telegram/"
...
async def image_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if file_extension == 'pdf':
        ...
        file_path = os.path.join(db_telegram_cache_path, update.effective_message.document.file_name)
        ...
        try:
            ...
            file_path = os.path.join(db_telegram_cache_path, file_name)
    ...
    await update_google_sheet(update, context, llm_response_text)

async def update_google_sheet(update, context, llm_response_text):
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # user intention analyses
    # rag = RAG()
    # result = rag.rag_pipeline(query = text, sheet_name = "Master Database", worksheet_name="partners")
    result = "OK" + text
...
if __name__ == '__main__':
    ...
    # Add text handler
    # text_handler_instance = MessageHandler(filters.Text, text_handler)

    application.add_handler(image_handler_instance)
    application.add_handler(pdf_handler_instance)
    # application.add_handler(text_handler_instance)
    # Start the polling
